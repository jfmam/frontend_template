...
이 폴더는 번들러 학습 실험실입니다. 먼저 mental model을 정리하고, 이후 Webpack처럼 동작하는 작은 번들러, Vite처럼 동작하는 작은 dev server, Rsbuild/Rspack식 incremental compiler를 직접 구현합니다.
...
4. `src/mini-rsbuild` - bundler dev server의 incremental compiler와 factory cache 구현
5. `docs/case-studies` - Rsbuild 전환, Vite build OOM, 이력서에 쓸 수 있는 설명 정리
